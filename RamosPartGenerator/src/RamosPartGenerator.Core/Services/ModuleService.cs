...
public sealed class ModuleService
{
    ...
    public IReadOnlyList<GeneratedPartRow> GeneratePreview(ModuleRequest request)
    {
        ...
        {
            throw new InvalidOperationException("Rev 30 Module?Ä I.C BrandÍ∞Ä Î∞òÎìú???ÑÏöî?©Îãà??");
        }

        if (isThirdParty && IsBlankCode(effectiveRequest.PurchaserCode))
        {
            throw new InvalidOperationException("Third-Party Module?Ä PurchaserÍ∞Ä Î∞òÎìú???ÑÏöî?©Îãà??");
        }
    ...
    public ModuleRequest ParseCompPart(string revision, string partCode)
    {
        ...
        if (parts.Length != 2)
        {
            throw new InvalidOperationException("Comp Full Part ?ïÏãù???¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§.");
        }
        ...
        {
            throw new InvalidOperationException("Comp Full Part Í∏∏Ïù¥Í∞Ä ?àÏÉÅ ?ïÏãùÍ≥?ÎßûÏ? ?äÏäµ?àÎã§.");
        }
        ...
        {
            throw new InvalidOperationException("Comp Full Part?êÏÑú Module???êÎèô Îß§Ìïë??ÎßåÎì§ ???ÜÏäµ?àÎã§.");
        }
    ...
    public ModuleRequest ParseModuleFullPart(string revision, string partCode)
    {
        ...
        {
            throw new InvalidOperationException("Module Full Part ?ïÏãù???¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§.");
        }
        ...
        {
            throw new InvalidOperationException($"Module Full Part ?ûÎ?Î∂?Í∏∏Ïù¥Í∞Ä ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§. ?ÑÏû¨ Í∏∏Ïù¥: {headPart.Length} / Í∏∞Î? Í∏∏Ïù¥: 11");
        }

        if (!string.IsNullOrEmpty(binTail) && binTail.Length != 7)
        {
            throw new InvalidOperationException("Module BIN suffix Í∏∏Ïù¥Í∞Ä ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§. ?? TNAGA00");
        }
        ...
        {
            throw new InvalidOperationException($"Rev {revisionSpec.DisplayRevision} Module tail Í∏∏Ïù¥Í∞Ä Î∂ÄÏ°±Ìï©?àÎã§.");
        }
        ...
        if (!string.IsNullOrEmpty(trailingText))
        {
            throw new InvalidOperationException($"Rev 30 Module tailø° «ÿºÆµ«¡ˆ æ ¿∫ ƒ⁄µÂ∞° ≥≤æ∆ ¿÷Ω¿¥œ¥Ÿ: {trailingText}");
        }
    ...
    private static string BuildModuleBasePartCode(ModuleRequest request)
    {
        ...
        {
            throw new InvalidOperationException("?†ÌÉù??Speed??ÎßûÎäî Module Bank/VDD ÏΩîÎìúÎ•?Í≥ÑÏÇ∞?????ÜÏäµ?àÎã§.");
        }
    ...
    private static void ValidateRequiredCodes(params string[] requiredCodes)
    {
        ...
        {
            throw new InvalidOperationException("Module ?ùÏÑ± ?ÑÏàò ÏΩîÎìúÍ∞Ä ÎπÑÏñ¥ ?àÏäµ?àÎã§.");
        }
    }
}
